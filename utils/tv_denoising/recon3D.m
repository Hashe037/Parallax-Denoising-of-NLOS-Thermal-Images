function [x_hat,out,opts] = recon3D(Y, model, opts)
% -----------------------------------
% USAGE:
%       ** Under construction **
% 
...
if opts.diagnostics                                                    
    fprintf('\n\n***********************************************************\n');
    fprintf('**    Entering 3D Regularized Reconstruction Algorithm   **\n');
    fprintf('***********************************************************\n');
    fprintf('\n SOLVING: \n')
...
for k=1:maxIter
    ...
    out.constraint_error(k) = gather(...
        ...
        norm(vec(d2 - D(x,2))) + ...
        norm(vec(d3 - D(x,3))) ) /...
        gather( norm( vec(D(x,1)) ) +...
                norm( vec(D(x,2)) ) + norm( vec(D(x,3)) ) );
...
out.status = ' ';
if convg
    out.status = sprintf('Success in %d iterations',k+start_iter);
else
